...
class SessionManager:
    ...
    def create_session(self, title: str, mode: str) -> Optional[Dict[str, Any]]:
        if not self.enabled:
            return None
    ...
    def append_speech_metrics(self, session_id: str, metrics: Dict[str, Any]) -> None:
        if not self.enabled:
            return
    ...
    def update_mode(self, session_id: str, mode: str) -> None:
        if not self.enabled:
            return
        self.client.table("sessions").update({"current_mode": mode, "updated_at": datetime.utcnow().isoformat()}).eq("id", session_id).execute()

    def update_context_markdown(self, session_id: str, context_md: str) -> None:
        if not self.enabled:
            return

        self.client.table("sessions").update({"context_md": context_md, "updated_at": datetime.utcnow().isoformat()}).eq("id", session_id).execute()

    def build_context_window(self, session_id: str, last_n: int = 8) -> Optional[SessionContextWindow]:
        session = self.get_session(session_id)
        if not session:
            return None

        recent = self.get_recent_messages(session_id, limit=max(1, last_n))
    ...
    def generate_session_summary_markdown(self, session_id: str) -> str:
    ...
    def _touch_session(self, session_id: str) -> None:
        if not self.enabled:
            return
        self.client.table("sessions").update({"updated_at": datetime.utcnow().isoformat()}).eq("id", session_id).execute()
